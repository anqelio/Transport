...
@router.delete('/route_stop/{route_stop_id}', status_code=status.HTTP_200_OK, description='Удаление остановки на маршруте')
...
@router.get('/route_stop', description='Вывод остановки на маршруте')
def router_show_employee(session: Session = Depends(get_session)):
    return show_route_stop(session)
...
def router_get_employee_by_id(employee_schedule_id: int, session: Session = Depends(get_session)):
    return get_employee_schedule_by_id(employee_schedule_id, session)
...
@router.delete('/employee_schedule/{employee_schedule_id}', status_code=status.HTTP_204_NO_CONTENT, description='Удаление графика рабочих')
def router_delete_employee(employee_schedule_id: int, session: Session = Depends(get_session)):
    return delete_employee_schedule_by_id(employee_schedule_id, session)


@router.put('/employee_schedule/{employee_schedule_id}', status_code=status.HTTP_200_OK, description='Изменение графика рабочих')
def router_update_employee(employee_schedule_id: int, data: EmployeeSchedules, session: Session = Depends(get_session)):
    return update_employee_schedule(employee_schedule_id, data, session)
...
def router_show_employee(session: Session = Depends(get_session), page: int = 1, size: int = 10):
    return show_employee_schedule(session, page, size)